 ...
 *)
 ...
 (* 将操作数（Const/Var/Temp/VReg）转换为字符串 *)
 let op_str = function
   | Const n -> string_of_int n
   | Var s -> s
   | Temp n -> "t" ^ string_of_int n
   | VReg n -> "v" ^ string_of_int n
 ...
 (* 表达式键类型 *)
 type expr_key = 
   | BinOpKey of Ast.binop * string * string
   | UnOpKey of Ast.unop * string
 ...
 type reg_alloc_state = {
   ...
   mutable var_map: (string, int) Hashtbl.t;
   mutable temp_map: (int, int) Hashtbl.t;
 }
 
 let create_reg_alloc () = {
   next_vreg = 0;
   var_map = Hashtbl.create 32;
   temp_map = Hashtbl.create 32;
 }
 
 let get_vreg_for_var alloc name =
   match Hashtbl.find_opt alloc.var_map name with
   | Some vreg -> VReg vreg
   | None ->
       let vreg = alloc.next_vreg in
       alloc.next_vreg <- vreg + 1;
       Hashtbl.add alloc.var_map name vreg;
       VReg vreg
 
 let get_vreg_for_temp alloc t =
   match Hashtbl.find_opt alloc.temp_map t with
   | Some vreg -> VReg vreg
   | None ->
       let vreg = alloc.next_vreg in
       alloc.next_vreg <- vreg + 1;
       Hashtbl.add alloc.temp_map t vreg;
       VReg vreg
 
 let clear_reg_alloc alloc =
   Hashtbl.clear alloc.var_map;
   Hashtbl.clear alloc.temp_map;
   alloc.next_vreg <- 0
 
 (* 将函数中的 Var 和 Temp 替换为 VReg *)
 let allocate_registers_func (f: ir_func) : ir_func =
   let alloc = create_reg_alloc () in
   
   let replace_op op =
     match op with
     | Var name -> get_vreg_for_var alloc name
     | Temp t -> get_vreg_for_temp alloc t
     | _ -> op
   in
   
   let replace_tac inst =
     match inst with
     | Assign (x, y) ->
         Assign (replace_op x, replace_op y)
     | AssignBinOp (x, op, y, z) ->
         AssignBinOp (replace_op x, op, replace_op y, replace_op z)
     | AssignUnOp (x, op, y) ->
         AssignUnOp (replace_op x, op, replace_op y)
     | Call (x, fname, n) ->
         Call (replace_op x, fname, n)
     | Return (Some x) ->
         Return (Some (replace_op x))
     | Param x ->
         Param (replace_op x)
     | _ -> inst
   in
   
   let replace_block (b: basic_block) =
     { b with instrs = List.map replace_tac b.instrs }
   in
   
   let new_entry = replace_block f.entry in
   let new_blocks = List.map replace_block f.blocks in
 ...
 (* 对整个程序做寄存器分配 *)
 let register_allocation (prog: ir_program) : ir_program =
   List.map (function
     | Function f -> Function (allocate_registers_func f)
     | GlobalVar _ as g -> g
   ) prog
...
(* 获取指令中使用的变量名集合 *)
let used_vars_of_tac_licm inst =
...
(* 获取指令定义的变量名 *)
let defined_var_of_tac_licm inst =
  match inst with
...
(* 获取基本块的所有后继标签 *)
let get_successors (blocks: basic_block list) (label: string) : string list =
  let rec find_block = function
    | [] -> failwith ("Block " ^ label ^ " not found")
    | b :: rest -> if b.label = label then b else find_block rest
  in
  let block = find_block blocks in
...
(* 查找循环头（检测回边）*)
let find_loop_headers (blocks: basic_block list) : (string * string) list =
  let is_back_edge from_label to_label =
    if from_label = to_label then true
    else
      (* 比较标签数字来判断回边：L0, L1, L2... *)
      try
        let from_num = int_of_string (String.sub from_label 1 (String.length from_label - 1)) in
        let to_num = int_of_string (String.sub to_label 1 (String.length to_label - 1)) in
        from_num > to_num
      with _ -> false
  in
  let rec collect acc = function
    | [] -> acc
    | b :: rest ->
        let succs = get_successors blocks b.label in
        let new_acc = List.fold_left (fun acc2 succ ->
          if is_back_edge b.label succ then
            (b.label, succ) :: acc2
          else
            acc2
        ) acc succs in
        collect new_acc rest
  in
  collect [] blocks

(* 获取循环中的所有基本块（从回边到循环头）*)
let get_loop_blocks (blocks: basic_block list) (header: string) (back_edge: string) : string list =
  let rec collect current acc =
    if List.mem current acc then acc
    else if current = header then current :: acc
    else
      let preds = List.filter (fun b ->
        List.mem current (get_successors blocks b.label)
      ) blocks in
      if preds = [] then current :: acc
      else
        let new_acc = current :: acc in
        List.fold_left (fun acc2 b ->
          collect b.label acc2
        ) new_acc preds
  in
  let result = collect back_edge [] in
...
let licm_optimize (prog: ir_program) : ir_program =
  let optimize_function (f: ir_func) =
    ...
    let loop_headers = find_loop_headers blocks in
    ...
    if loop_headers = [] then f
      ...
      let process_block b =
        ...
        | Some (_, back_edge) ->
            (* 收集循环中定义的变量 *)
            let loop_blocks = get_loop_blocks blocks b.label back_edge in
            let loop_defs = ref VarSet.empty in
            
            List.iter (fun label ->
              try
                let bb = List.find (fun x -> x.label = label) blocks in
                List.iter (fun inst ->
                  match defined_var_of_tac_licm inst with
            ...
            let rec scan acc = function
              ...
              | inst :: rest ->
                  if is_side_effect_free inst then
                    (let used = used_vars_of_tac_licm inst in
      ...
      (* 将移动的指令插入到 entry 块前面 *)
      let final_blocks = List.map (fun b ->

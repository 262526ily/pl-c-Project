 ...
 *)
 ...
 let rec eval_const_expr st = function
   | Ast.EInt n -> Some n
   | Ast.EId name ->
       (try 
          match find name st.sym_table with
          | Some { kind = Const; _ } -> Const_table.get_const_value name
          | _ -> None
        with Not_found -> None)
   | Ast.EBinOp (op, l, r) ->
       (match eval_const_expr st l, eval_const_expr st r with
        | Some a, Some b ->
            Some (match op with
              | Ast.Add -> a + b
              | Ast.Sub -> a - b
              | Ast.Mul -> a * b
              | Ast.Div -> if b = 0 then failwith "div by zero" else a / b
              | Ast.Mod -> if b = 0 then failwith "mod by zero" else a mod b
        ...
        | _ -> None)
   | Ast.EUnOp (op, e) ->
       (match eval_const_expr st e with
        | Some n ->
            Some (match op with
              | Ast.Pos -> n
              | Ast.Neg -> -n
 ...
 let check_decl st (d: Ast.decl) : unit =
   ...
   | Ast.ConstDecl (name, init) ->
       if find_current name st.sym_table <> None then
         ...
         (* 计算并记录常量值到 Const_table *)
         (match eval_const_expr st init with
          | Some v -> Const_table.record_const_value name v
          | None -> ());

...
let emit_tac fname tac_inst map current_args =
  ...
  | AssignBinOp (x, op, y, z) ->
      (match op with
       (* ================= 乘法优化：常数乘变移位 ================= *)
       | Ast.Mul ->
           let try_power_of_two y_op z_op =
             match y_op, z_op with
             | Const n, _ when is_power_of_two n && n > 1 ->
                 load_op "t0" z_op map;
                 Printf.printf "    slli t0, t0, %d\n" (log2 n);
                 store_op "t0" x map; true
             | _, Const n when is_power_of_two n && n > 1 ->
                 load_op "t0" y_op map;
                 Printf.printf "    slli t0, t0, %d\n" (log2 n);
                 store_op "t0" x map; true
             | _ -> false
           in
           if not (try_power_of_two y z) then
             (* 普通乘法：使用内联乘法 *)
       ...
       (* ================= 除法优化：常数除变移位 ================= *)
       | Ast.Div ->
           let try_power_of_two_div y_op z_op =
             match z_op with
             | Const n when is_power_of_two n && n > 1 ->
                 load_op "t0" y_op map;
                 Printf.printf "    srai t0, t0, %d\n" (log2 n);
                 store_op "t0" x map; true
             | _ -> false
           in
           if not (try_power_of_two_div y z) then
             (* 普通除法：使用内联除法 *)
       ...
       (* ================= 取模优化：常数取模变与运算 ================= *)
       | Ast.Mod ->
           let try_power_of_two_mod y_op z_op =
             match z_op with
             | Const n when is_power_of_two n && n > 1 ->
                 load_op "t0" y_op map;
                 Printf.printf "    addi t1, x0, %d\n" (n - 1);
                 Printf.printf "    and t0, t0, t1\n";
                 store_op "t0" x map; true
             | _ -> false
           in
           if not (try_power_of_two_mod y z) then
             (* 普通取模：使用内联取模（复用除法逻辑） *)
             let lbl_start  = gen_inline_label ".L_divmod_start" in
             let lbl_n_pos  = gen_inline_label ".L_divmod_n_pos" in
             let lbl_d_pos  = gen_inline_label ".L_divmod_d_pos" in
